...
#=============================================================================
#Reading CSV file
df_credit <- read.csv("dcc.csv", stringsAsFactors = FALSE)
#df <- df_credit
...
df_credit$SEX <- factor(df_credit$SEX,
                        level = c(1,2),
                        labels = c("Male", 
                                   "Female"))
#Data Cleaning Education column
df_credit$EDUCATION[df_credit$EDUCATION == 0] <- 4
df_credit$EDUCATION[df_credit$EDUCATION == 5] <- 4
df_credit$EDUCATION[df_credit$EDUCATION == 6] <- 4
#Converting to factor and labelling Education column
df_credit$EDUCATION <- factor(df_credit$EDUCATION,
                              level = c(1,2,3,4),
...
df_credit$MARRIAGE[df_credit$MARRIAGE == 0] <- 3 
#Converting to factor and labelling Marriage column
df_credit$MARRIAGE <- factor(df_credit$MARRIAGE,
                             level = c(1,2,3),
                             labels = c("Married",
                                        "Single",
                                        "Others"))
#Converting default payment column to factor
df_credit$default.payment.next.month <- as.factor(df_credit$default.payment.next.month)
#Converting the Pay_N columns to factor
df_credit[,c(7:12)] <- lapply(df_credit[,c(7:12)],factor)
#==========================================================================
#==============================================================================
...
ggplot(df_credit, aes(EDUCATION, fill=EDUCATION))+geom_bar(position='dodge', )+ geom_text(stat='count',
                                                                                          aes(label = ..count..),
                                                                                          position = position_dodge(width = 1.0), 
                                                                                          vjust = -0.5)
#2. Limit Balance of Customers by Education (Box Plot)
ggplot(df_credit, aes(EDUCATION, LIMIT_BAL)) +  geom_boxplot(notch = TRUE, fill = c("green", "red", "orange", "pink"))
#3. Number of Customers who will default ot not depending on Education
ggplot(df_credit, aes(x= EDUCATION,  group=default.payment.next.month)) + geom_bar(aes(y = ..prop.., fill = factor(..x..)), stat="count") + 
...
#4.Marriage Group BY
ggplot(df_credit, aes(MARRIAGE, fill=MARRIAGE))  + geom_bar(position="dodge") + geom_text(stat='count', aes(label = ..count..),position = position_dodge(width = 1.0), vjust = -0.5)
#5.Marriage Default Payment
ggplot(df_credit, aes(MARRIAGE,fill=default.payment.next.month))  + geom_bar(position="dodge", ) + geom_text(stat='count', aes(label = ..count..),position = position_dodge(width = 1.0), vjust = -0.5)
#6.Marriage percentage based on default
ggplot(df_credit, aes(x= MARRIAGE,  group=default.payment.next.month)) + geom_bar(aes(y = ..prop.., fill = factor(..x..)), stat="count") + 
  geom_text(aes( label = scales::percent(..prop..), y= ..prop.. ), stat= "count", vjust = -.5) +
  labs(y = "Percent", fill="Marriage") +facet_grid(~default.payment.next.month) +scale_y_continuous(labels = scales::percent)

#7. Sex vs Default
ggplot(df_credit,aes(SEX,fill=SEX)) + geom_bar()  + geom_text(stat='count', aes(label = ..count..),position = position_dodge(width = 1.0), vjust = -0.5)
#8. Sex vsLimit Balance
ggplot(df_credit, aes(x=df_credit$LIMIT_BAL, fill=SEX, facet=SEX)) + geom_histogram(position="identity", alpha=0.5)+
  facet_grid(~df_credit$SEX)+scale_x_continuous(breaks = seq(0, 1000000, by=50000))+theme(axis.text.x = element_text(angle = 45, hjust=1))
#9. Sex vs Default
ggplot(df_credit, aes(x= default.payment.next.month,  group=SEX)) + 
...
ggplot(df_credit, aes(AGE, LIMIT_BAL)) + geom_point(aes(color=df_credit$default.payment.next.month)) + geom_smooth(method=lm)
#11. Age vs Default
ggplot(df_credit, aes(x=AGE, fill=default.payment.next.month)) + geom_histogram(position="identity", alpha=0.5)+
...
rows <- nrow(df_credit)
train<-df_credit[rows, ]
split <- round(nrow(df_credit) * 0.60)
train.df <- df_credit[1:split, ]
test.df <-  df_credit[(split + 1):rows, ]
# Confirm test set size
nrow(train.df) / nrow(df_credit)
library(caret)
...
prop.table(table(df_credit$default.payment.next.month)) * 100
